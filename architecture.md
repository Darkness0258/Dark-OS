...
## Key decisions log
- 2026-08-23: Implemented the Command Center split — dock + rail stay always-on, `DarkOSHUDOverlay` + left + right panels now start hidden and open together via `--toggle-command-center` (SUPER+H — SUPER+C was already `killactive`). While wiring this, found `DarkOSHUDOverlay` was defined and imported but **never instantiated** — Phase 2's "Central AI Core HUD: done, VM-verified 2026-08-16" checkbox didn't match what actually ran. It's also still a text-only "PREVIEW MODE" stub, not the ring-graphic HUD ui-rules.md/ui-registry.md describe (that graphic currently lives in the static wallpaper). Fixed the wiring gap; the real Cairo ring HUD is still unbuilt — see build-plan.md Phase 2/3. Also added two feature commitments to Phase 5 (not yet implemented — no Phase 5 code exists): Network Center's Connect tab gets real KDE-Connect-protocol phone integration (Zorin Connect-inspired), and Settings > System gets a kernel/scheduler + CPU governor Performance profile (CachyOS-inspired), folded into Phase 8's first-run flow.
