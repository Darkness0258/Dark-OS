...
wait_for_ready() {
    ...
    while (( SECONDS - start < timeout )); do
        ...
        elif [[ "$layers" != *'namespace: darkos-left'* ]]; then
            reason='darkos-left layer is absent'
        elif [[ "$layers" != *'namespace: darkos-right'* ]]; then
            reason='darkos-right layer is absent'
...
assert_runtime_health() {
    ...
    for namespace in darkos-dock darkos-rail darkos-left darkos-right; do
