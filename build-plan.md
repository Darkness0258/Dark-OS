...
- [x] Calamares install wizard completes in a VM (partition, unpackfs, user/root password, bootloader, reboot prompt) — not yet confirmed the installed system actually boots or that the passwordless live-session setup is gone. `darkos-tool-groups` picker script not yet written.
