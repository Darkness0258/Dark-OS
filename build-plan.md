...
- [x] "Explain this" **code wired** — `ActionDispatcher.explain()` pulls AT-SPI selected text or active window title, returns it for the brain to explain inline. `[ ]` Runtime verified in live VM: select error text, ask "explain this", confirm AI explains it.
- [x] Context-aware shell **code wired** — `ActivityDetector` started in `do_activate()` with `_on_activity_changed` listener. Swaps dock icon highlight and panel visibility per profile (coding/gaming/writing/media/default). `[ ]` Runtime verified in live VM: switch between coding app and game, watch layout change.
