...
Status: refactor done 2026-08-18. Wiring + new modules completed 2026-08-20. All py_compile clean. Container runtime verification suite passed 2026-08-20 (`ci/test-phase3-linux.py`). **2026-08-20 code review & container runtime test run found and fixed 3 real bugs** (see items below).
...
- [x] STT + TTS + brain **code wired** — `ai_brain.py` wired to chat entry and voice trigger. Chat `on_submit` calls `brain.process_chat()` on a background thread; voice dispatch routes through `process_voice → process_chat → speak`. Orb state transitions (listening/thinking/speaking/error/sleeping) driven by the dispatch chain. Container test verified audio pipeline routes & player selection. `[ ]` Runtime verified in live VM: say wake word → audible response, type in chat → response.
- [x] OS-level control via D-Bus / hyprland **code wired** — `actions.py:ActionDispatcher` built with `open_app`, `set_volume`, `set_brightness`, `switch_workspace`, `search`. Brain dispatches via `[ACTION]` markers in LLM replies. Snapshot-before-act wraps all mutating actions. **Bug fixed 2026-08-20:** `_parse_args` split on every comma, so any argument text containing one (e.g. `open_app` with a comma in it) mis-split into extra args and errored — rewritten to use `csv` parsing, which respects commas inside quotes. Container test verified before/after state on volume & workspace switches. `[ ]` Runtime verified in live VM: ask to open an app / change volume and confirm it executes.
- [x] Generic in-app control via AT-SPI **code wired, was actually broken 100% of the time** — `actions.py:_atspi_do_action` walks the AT-SPI tree supporting click, set_text, focus actions. **Bug found and fixed 2026-08-20:** the subprocess call was `["python3", "-c", script, "--", payload]` — Python's `-c` mode puts that literal `"--"` into `sys.argv[1]`, not the payload, so `json.loads(sys.argv[1])` crashed with `JSONDecodeError` on every single call, silently, before the tree walk ever ran (confirmed by actually reproducing the exact failure, not just reading the code). Removed the stray `"--"` and verified the subprocess now receives and parses the payload correctly. `_atspi_get_selected_text`/`_atspi_get_active_window_text` (the explain-this path) don't pass this arg, so they weren't affected. Container test verified action payload serialization & node matching. `[ ]` Runtime verified in live VM: proven on at least 2 apps per architecture.md.
...
- [x] Snapshot-before-act **code wired** — `actions.py:_snapshot()` creates Btrfs snapshots at `/.snapshots/darkos-ai-<timestamp>` before any mutating action. Silently skips on non-Btrfs. **Bug found and fixed 2026-08-20:** destination path had an invalid literal `@` (`/@/.snapshots/...`) instead of `/.snapshots/...`, and missing directory creation before snapshotting. Fixed with `Path(self._snapshot_root) / ".snapshots"` and `mkdir(parents=True, exist_ok=True)`. Container test verified snapshot creation and `btrfs subvolume list` output. `[ ]` Runtime verified on installed bare metal / VM Btrfs root.
- [x] "Explain this" **code wired** — `ActionDispatcher.explain()` pulls AT-SPI selected text or active window title, returns it for the brain to explain inline. Container test verified window error text extraction. `[ ]` Runtime verified in live VM: select error text, ask "explain this", confirm AI explains it.
- [x] Context-aware shell **code wired** — `ActivityDetector` started in `do_activate()` with `_on_activity_changed` listener. Swaps dock icon highlight and panel visibility per profile (coding/gaming/writing/media/default). Container test verified classification on 5 app types. `[ ]` Runtime verified in live VM: switch between coding app and game, watch layout change.
