...
- **Installed system has NO autologin** — `users.conf` sets `doAutologin: false`. The live ISO auto-logs-in via the `getty@tty1` autologin override (separate from Calamares), but a real install prompts for the user password at boot. If the product decides boot-to-desktop is wanted on installed systems, re-enable `doAutologin` deliberately.
