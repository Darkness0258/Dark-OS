...
- Used in: `darkos-settings.py`, `darkos-network.py`; the rail's "settings" action launches Settings directly. Network Center's new Activity tab (network broker visibility layer, 2026-09-17) reads `/var/log/darkos/network-activity.json`, written by `darkos-netwatch.py` — no subprocess call, so it polls the file every 5s via `GLib.timeout_add_seconds` rather than using `_run_async`.
- Notes: `darkos_shell/user_settings.py` is the new shared read/write layer — `tokens.py` imports it at module load, so accent color, corner radius, and reduce-motion are live values with hardcoded fallbacks, not a write-only JSON file. Confirmed by direct test (write settings.json, re-import tokens, values change) and by wiring `REDUCE_MOTION` into the HUD's own tick handler as the first real consumer. Two configparser gotchas (default key-lowercasing, and `write()`'s spacing not matching this repo's `.desktop` convention) were caught and fixed in Startup's toggle before shipping — see build-plan.md Phase 5 for detail. Activity tab real-rendered under Xvfb (mocking `darkos_shell.app_kit` the same way `ci/test-app-kit.py` already does, to skip `surfaces.py`'s GtkLayerShell dependency, which this tab has nothing to do with): sample data sorted first-seen-first and labeled correctly, and the missing-snapshot-file fallback (service not running) renders its own status message plus a working Refresh button. New CSS: `.flag-text` (`color-warning`, 600 weight) for first-time-connection rows, reusing the existing `.eyebrow`/`.stub-text` one-rule pattern — no new token needed.
